...
export const Diagram = ({kind}: IDiagram) => {
  ...
  return (
    <div className={ [styles['diagram'], styles[`diagram_kind_${kind}`] ].join(' ')}>
      <div className={ [styles['textWrapper'], styles[`textWrapper_kind_${kind}`] ].join(' ')}>
        <p className={ [styles['text'], styles[`text_kind_${kind}`] ].join(' ')}>Веб-конструкторы</p>
